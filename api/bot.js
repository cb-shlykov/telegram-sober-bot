  ...
  try {
    switch (callbackData) {
      case 'start_today':
        ...
        // Удаляем инлайн-клавиатуру и показываем подтверждение в ОДНОМ сообщении
        ...
        await ctx.reply('Теперь у тебя есть кнопка для сброса счетчика:', {
          reply_markup: {
            keyboard: [
              [{ text: '🔄 Сбросить счетчик' }]
            ],
      ...
      case 'input_days':
        // Удаляем инлайн-клавиатуру и просим ввести количество дней
        await ctx.editMessageText(
...
bot.on('text', async (ctx) => {
  ...
  if (/^\d+$/.test(text)) {
    ...
    try {
      ...
      // Отправляем ОДНО сообщение с подтверждением и информацией
      let responseText = `📊 Зафиксировано: ${daysCount} дней без алкоголя! 🎉\n\n` +
        `Это огромное достижение! Продолжай в том же духе!\n\n`;

      // Добавляем сообщение для текущего дня, если есть
      const message = await airtableService.getMessageForDay(daysCount);
      if (message) {
        responseText += `💫 Изменения за ${daysCount} дней:\n\n${message}\n\n`;
      }

      responseText += `💪 Продолжайте в том же духе! Я буду присылать вам информацию о позитивных изменениях в вашем организме.\n\n` +
